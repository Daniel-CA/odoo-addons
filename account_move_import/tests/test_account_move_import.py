...
class TestAccountMoveImport(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.journal_id = self.env['account.journal'].create(
            {'name': 'Bank-test', 'type': 'bank', 'code': 'TEST'})
        ...
        file = open(path1, 'r')
        # data = [x.decode('utf8').strip() for x in file.readlines()]
        # data = base64.encodestring(file.read())
        data = base64.encodebytes(file.read().encode('utf-8'))
        file.close()
        wiz_vals = {'data': data,
                    'name': 'move_data.csv',
                    'delimeter': ';'}
    ...
    def test_exception_raise(self):
        wiz_error3_vals = {
            'data': base64.encodebytes(
                ...
                u'X11006;X11006;;600;;;12-31-2016;'.encode('utf-8')),
            'name': 'data_file.csv',
            'delimeter': ';'}
